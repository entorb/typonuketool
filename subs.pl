...
sub openFile {
  ...
  close FH;
  @cont
      = map { s/\s+$//; $_ } @cont; # remove whitespaces and \n from line ends
...
sub detectEncoding {
  ...
  # detects/guesses encoding 'utf-8' or 'iso-8859-1'
  my $cont = shift;
...
sub clearLatex {
  ...
  $cont =~ s/\\(,| )/ /g;                   # "\," und "\ "
  $cont
      =~ s/\\begin\{(thebibliography|tabular|equation|align|verbatim|titlepage)\}.*?\\end\{\1\}//sg
      ;    # totally remove some environments (+ contents)
  ...
  $cont
      =~ s/\s*\\(documentclass|usepackage|include|input|includegraphics|index|label|todo|todof)[^\}]*\}//mg
      ;    # + arguments of this commands

  # not a good idea!  $cont =~ s/\\\w+//g; # all commands

  # remove some latex commands but keep arguments
  @_
      = qw(cBild cBildDraw section subsection subsubsection chapter paragraph subparagraph newpage text\w{2} emph hline item footnote todocite klau klauPd rm text edit change);
  $_ = join '|', @_;
  $cont =~ s/\\($_)(\[|\{)/$2/g;

  # replace all refs by Xref
  @_
      = qw(cite ref tag pageref eqnref eqref autoref secref figref Figref tabref secrefPage figrefPage FigrefPage eqnrefPage tabrefPage);
  $_ = join '|', @_;
...
sub printResultLineWordCaseInsensitive {
  ...
  $s = "<tr><td>$sTreffer</td><td>\n$line2\n</td></tr>\n";
  print encode ( $opt{encodingOutput}, $s );
} ## end sub printResultLineWordCaseInsensitive
...
sub findDoppelteWoerter {
  ...
  for ( my $i = 0; $i <= $#cont; $i++ ) {
    ...
    # wort1 wort 1
    while ( $line =~ m/(((?<!\\)\b[[:alpha:]]+)\s+\2\b)/ig )
    {    # ignores \EA EA
      my $sTreffer = $1;
      $hits++;
      if ( $hits == 1 ) {printResultsHeader}
    ...
    while ( $line
      =~ m/(((\b[[:alpha:]]+)\s+([[:alpha:]]+)\s+([[:alpha:]]+)\b)\s+\2\b)/ig
        )
    {
      my $sTreffer = $1;
      $hits++;
    ...
    while ( $line
      =~ m/(((\b[[:alpha:]]+)\s+([[:alpha:]]+)\s+([[:alpha:]]+)\s+([[:alpha:]]+)\b)\s+\2\b)/ig
        )
    {
      my $sTreffer = $1;
      $hits++;
...
sub findKleinNachPunkt {
  ...
  for ( my $i = 0; $i <= $#cont; $i++ ) {
    ...
    while ( $line
      =~ m/(\b[[:alpha:]][[:alpha:]]+(\.|!|\?) [[:lower:]][[:alpha:]]+\b)/g )
    {
      my $sTreffer = $1;
      next if $sTreffer =~ m/(iapp\.de|dx\.doi|www\.iapp)/;
      my @l
          = qw(bzw. ca. cf. etc. vgl. no. m.E. m.a.W. u.U. s.u. z.B. al. lat. vs.)
          ;    # ignore these
      $_ = join "|", @l;
      s/\./\\./g;
  ...
  for ( my $i = 0; $i <= $#cont; $i++ ) {
    ...
    if ( $opt{language} eq 'EN' ) {
      while ( $line =~ m/(\d+,\d+)/g ) {
        my $sTreffer = $1;
        next
            if ( $line =~ m/($sTreffer,|,$sTreffer)/ )
            ;    # ignore if an additional commer follows.
      ...
      while ( $line =~ m/(\d+\.\d+)/g ) {
        ...
        next
            if ( $line =~ m/($sTreffer\.|\.$sTreffer)/ )
            ;    # ignore if an additional point follows.
...
sub findLeerzeichen {
  ...
  if ( $opt{language} eq 'DE' ) {
    ...
    $info
        = "Mehrfache überflüssige Leerzeichen, die in Word or LibreOffice zu unschönen Wortabständen führen würden.";
  }
  else {
...
# Im Latex Mode werden [ und  { rausgelöscht, daher nur relevant für copy&paste...
    ...
    if ( $Klammernzaehler[0] > 0 ) {
      ...
      if ( $hits == 1 ) {printResultsHeader}
      printResultLine( $lineNr, $sTreffer, $line );
    } ## end if ( $Klammernzaehler[...])
    elsif ( $Klammernzaehler[0] < 0 ) {
      my $sTreffer = ')';
      $hits++;
      if ( $hits == 1 ) {printResultsHeader}
      printResultLine( $lineNr, $sTreffer, $line );
    } ## end elsif ( $Klammernzaehler[...])
...
sub findWordsToAvoid {
  ...
  for ( my $i = 0; $i <= $#cont; $i++ ) {
    ...
    if ( $opt{username} =~ m/torben/i ) {
      while ( $line =~ m/\b(IPs?|Analog|Opposite|x-ray)\b/g )
      {    # IE, Analogously, Contrary, X-ray
        my $sTreffer = $1;
        $hits++;
        if ( $hits == 1 ) {printResultsHeader}
        printResultLineWord( $lineNr, $sTreffer, $line );
...
sub findTorbensLieblingsfehler {
  ...
  if ( $opt{language} eq 'DE' ) {
    ...
    $info
        = "Worte die Torben gerne falsch schreibt, wie 'der selbe' das auseinander geschrieben wird.";
  }
  else {
    printTitle('Torben\'s Favorite Mistakes');
    $info
        = "Some words I usually misspell, e.g. can not -> cannot, form -> from., effected -> affected, weather -> whether, molar -> molecular ...";
  }
  ...
  for ( my $i = 0; $i <= $#cont; $i++ ) {
    ...
    while ( $line =~ m/\b($s)\b/ig ) {
      my $sTreffer = $1;
      next
          if ( $sTreffer =~ m/form/i
        and $line =~ m/(to|which|do not|another) form/i );
      next
          if ( $sTreffer =~ m/molar/i
        and $line =~ m/molar (mass|masses|ratio)/i );
      next
          if ( $sTreffer =~ m/trends/i
        and $line =~ m/(the|to|these) trends/i );
      next
          if ( $sTreffer =~ m/to strong/i
        and $line =~ m/due to strong/i );
    ...
    @l = ();
    if ( $opt{language} eq 'EN' ) {
      push @l, ('This is a Placeholder');
    }
    elsif ( $opt{language} eq 'DE' ) {
      push @l, (
        '[Aa]m Besten', '[Zz]um Einen',
        '[Zz]um Anderen'    # einen anderen
      );
    } ## end elsif ( $opt{language} eq...)
    ...
    if ( $opt{language} eq 'EN' ) {
      while ( $line =~ m/\b(this +\w+s)\b/ig ) {
        ...
        next
            if ( $sTreffer
          =~ m/this (agrees|allows|cancels|corresponds|enables|explains|hypothesis|leads|shows|suggests|thesis)/i
            );
        $hits++;
        if ( $hits == 1 ) {printResultsHeader}
        printResultLineWord( $lineNr, $sTreffer, $line );
...
sub findLatexProbs {
  ...
  if ( $opt{language} eq 'DE' ) {
    ...
    $info
        = "Einheiten nicht kursiv und mit '\\,' als Abstand zur Zahl (Empfehlung: Paket siunitx). Wörter/Kürzel in Subscripten nicht kursiv: E_{act} -> E_\\text{act}. Eine Übersicht über veraltete Pakete gibt <a href='ftp://ftp.rrzn.uni-hannover.de/pub/mirror/tex-archive/info/l2tabu/german/l2tabu.pdf'>l2tabu.pdf</a>.";
  }
  else {
    printTitle('Latex Stuff');
    $info
        = "Units not in italics and with '\\,' as space to number (best use package siunitx). Words in subscripts not in italics: E_{act} -> E_\\text{act}. For obsolete packages see <a href='ftp://ftp.rrzn.uni-hannover.de/pub/mirror/tex-archive/info/l2tabu/english/l2tabuen.pdf'>l2tabu.pdf</a>.";
  }
  for ( my $i = 0; $i <= $#cont; $i++ ) {
    my $line   = $cont[$i];
    my $lineNr = $i + 1;
    ...
    while ( $line =~ m/(\d ?(nm|mm|cm|m|eV|meV|W|mW|kW|J|kJ)\$)/g ) {
      ...
      if ( $hits == 1 ) {printResultsHeader}
      printResultLine( $lineNr, $sTreffer, $line );
    } ## end while ( $line =~ m/(\d ?(nm|mm|cm|m|eV|meV|W|mW|kW|J|kJ)\$)/g)
    ...
    while ( $line =~ m/(\\begin\{(eqnarray|appendix)\})/g ) {
      ...
      if ( $hits == 1 ) {printResultsHeader}
      printResultLine( $lineNr, $sTreffer, $line );
    } ## end while ( $line =~ m/(\\begin\{(eqnarray|appendix)\})/g)
    ...
    while ( $line =~ m/(\{scrlettr|SIunits\})/g ) {
      ...
      if ( $hits == 1 ) {printResultsHeader}
      printResultLine( $lineNr, $sTreffer, $line );
    } ## end while ( $line =~ m/(\{scrlettr|SIunits\})/g)

    # Packages
    while ( $line
      =~ m/(\\usepackage.*\{(psfig|doublespace|fancyheadings|scrpage|isolatin1|umlaut|t1enc|subfig|times|mathptm|pslatex|palatino|mathpple|utopia|pifont|euler|ae|zefonts)\})/g
        )
    {
      my $sTreffer = $1;
      $hits++;
    ...
    while ( $line =~ m/(\\bibliographystyle\{dinat\})/g ) {
      ...
      if ( $hits == 1 ) {printResultsHeader}
      printResultLine( $lineNr, $sTreffer, $line );
    } ## end while ( $line =~ m/(\\bibliographystyle\{dinat\})/g)
  }    # foreach line
  if ($hits) {
    printResultsFooter;
    printInfo($info);
...
sub findPunctuation {
  ...
  if ( $opt{language} eq 'DE' ) {
    ...
    $info
        = "Comma after words like 'Hence, Therefore, Thus' if they are opening the sentence.";
  }
  for ( my $i = 0; $i <= $#cont; $i++ ) {
    my $line   = $cont[$i];
    my $lineNr = $i + 1;
    ...
    if ( $opt{language} eq 'EN' ) {
      ...
      push @after, (
        'In the following', 'In this (thesis|work)'
    ...
    @after  = map { decode( 'utf-8', $_ ) } @after;
    @after  = sort @after;
    @before = map { decode( 'utf-8', $_ ) } @before;
    @before = sort @before;
    if (@after) {
      my $s = join '|', @after;
      while ( $line =~ m/(\b($s) )/g ) {    # case sensitive
        my $sTreffer = $1;
        next
            if ( $sTreffer =~ m/First/i
          and $line =~ m/First (attempts)/i );
        next
            if ( $sTreffer =~ m/Analogously/i
          and $line =~ m/Analogously (to)/i );
        next
            if ( $sTreffer =~ m/Contrary/i
          and $line =~ m/Contrary (to)/i );
        next
            if ( $sTreffer =~ m/In the following/i
          and $line =~ m/In the following (section|chapter)s?/i );
        $hits++;
        if ( $hits == 1 ) {printResultsHeader}
        printResultLine( $lineNr, $sTreffer, $line );
      }    # while
      while ( $line
        =~ m/(\b(In (this|the following|the next|the previous) (chapter|section|thesis)s?) )/g
          )
      {    # case sensitive
        my $sTreffer = $1;
        next
    ...
    if ( $opt{language} eq 'EN' ) {
      ...
      # no komma here
      while ( $line =~ m/(,\s+((for|in) (devices|samples), where))/ig )
      {    # case insensitive
        my $sTreffer = $1;
        $hits++;
        if ( $hits == 1 ) {printResultsHeader}
...
sub findBindestrich {
  ...
  my $tStart = time();
  my $cont   = "\L$cont2"
      ; # Lower case und sicherstellen, dass das original nicht verändert wird
  ...
  if ( $opt{language} eq 'DE' ) {
    ...
    printTitle('Hyphens');
    $info
        = "'temperature-independant' vs. 'temperature independant' vs. 'temperatureindependant'";
  }
  ...
  } ## end while ( $cont =~ m/([[:alpha:]]+)\-([[:alpha:]]+)/gi)
  ...
  } ## end foreach my $wMit ( keys(%BindestrichWoerter...))
...
# nur 1x gleichzeitig mit vielen Suchbegriffen durch den Inhalt schleifen ist viel schneller als pro Wort 3x schleifen!!!
  ...
  foreach my $wMit ( keys(%BindestrichWoerter) ) {
    ...
    $wZ =~ s/\-//;
    $BindestrichFehler{$wMit}++
        if ( $BindestrichWoerterOhne{$wOhne} > 0 );
    $BindestrichFehler{$wMit}++
        if ( $BindestrichWoerterZusammen{$wZ} > 0 );
  } ## end foreach my $wMit ( keys(%BindestrichWoerter...))
  ...
  foreach my $wMit ( keys %BindestrichFehler ) {
    ...
    $wZ    =~ s/\-//;
    print
        "$wMit = <font color='red'><b>$BindestrichWoerter{$wMit}x</b></font>";
    print
        " ; $wOhne = <font color='red'><b>$BindestrichWoerterOhne{$wOhne}x</b></font>"
        if $BindestrichWoerterOhne{$wOhne};
    print
        " ; $wZ = <font color='red'><b>$BindestrichWoerterZusammen{$wZ}x</b></font>"
        if $BindestrichWoerterZusammen{$wZ};
    print "<br>\n";
  } ## end foreach my $wMit ( keys %BindestrichFehler)
  @_    = keys(%BindestrichFehler);
  $hits = 1 + $#_;
  @_    = ();
...
sub findAvsAn {
  ...
  if ( $opt{language} eq 'DE' ) {
    ...
    printTitle('A vs An');
    $info
        = "right: 'a house', 'an example'<br>wrong: 'an european', 'an UHV', 'a LED'\n";
  }
  for ( my $i = 0; $i <= $#cont; $i++ ) {
    my $line   = $cont[$i];
    my $lineNr = $i + 1;
...
sub findWortwiederholungen {
  ...
  while ( my $wort = lc( shift(@woerter) ) ) {
    ...
    while ( $i <= $maxAbstand and $next = lc( shift @woerterTemp ) ) {
      ...
      $i++;
    } ## end while ( $i <= $maxAbstand...)
  } ## end while ( my $wort = lc( shift...))
  ...
  if ( $opt{timing} == 1 )
  {    # print ranking of repeated words only when in debug mode
    print "Ranking<br\n>";
    foreach my $key (
      sort { $wortwiederholungsranking{$b} <=> $wortwiederholungsranking{$a} }
      keys %wortwiederholungsranking
        )
    {
      ...
      print $_;
    } ## end foreach my $key ( sort { $wortwiederholungsranking...})
  } ## end if ( $opt{timing} == 1)
...
sub ZaehleSatzWortSilbe {
  ...
  $cont =~ s/\b[^Ia]\b/ /g
      ;    # remove 1-char words # do care about word "I", "a" ;-)
  ...
  my $lengthInMeter
      = sprintf( "%.1f", $size / (80) * 0.15 ); # 80 chars/line, 15cm per line
  my $proofReadingDuration = sprintf "%d",
      ( $size * ( 54.5 / 25000 ) );    # gestoppt in Diss, Rechnungskapitel
...
$opt{'percentThe'} % of sentences starting with '"
      . ( $opt{language} eq 'EN' ? "The" : "Der, Die, Das, Den" ) . "'<br>
$numWords     words<br>
...
\n";
  ...
  if ( $numSentences > 2 and $numWords > 2 ) {
    my $avWordsPerSentence
        = $numWords / $numSentences;    # Average Sentence Length
    my $avSyllables
        = $numSyllables / $numWords;    # Average Number of Syllables per Word
    ...
    if ( $opt{language} eq 'EN' ) {
      $fre = 206.835 - ( 1.015 * $avWordsPerSentence )
          - ( 84.6 * $avSyllables );
      print
          "<a target='blank' href='http://en.wikipedia.org/wiki/Flesch-Kincaid_readability_test#Flesch_Reading_Ease'>";
      printf( "%.1f", $fre );
      print "</a>";
    } ## end if ( $opt{language} eq...)
    elsif ( $opt{language} eq 'DE' ) {
      ...
      print
          "<a target='blank' href='http://de.wikipedia.org/wiki/Lesbarkeitsindex#Flesch_Reading_Ease'>";
      printf( "%.1f", $fre );
      print "</a> <small>(higher means easier)</small>\n";
    } ## end elsif ( $opt{language} eq...)
  } ## end if ( $numSentences > 2...)
  print "</p\n>";
...
sub countPhrases {
  ...
  foreach my $satz (@saetze) {
    ...
    for ( my $i = 0; $i <= $#woerter - 1; $i = $i + 2 ) {
      ...
      next
          if ( length $woerter[ $i + 1 ] < 2 );   # words at least 2 char long
    ...
    for ( my $i = 0; $i <= $#woerter - 2; $i = $i + 3 ) {
      ...
      next
          if ( length $woerter[ $i + 1 ] < 2 );   # words at least 2 char long
      next
          if ( length $woerter[ $i + 2 ] < 2 );   # words at least 2 char long
  ...
  if ( $opt{language} eq 'DE' ) {
    ...
    @_ = (
      'and the', 'as the', 'at the', 'by the',
      'for the', 'in the', 'is the', 'it is',
      'of the',  'of is',  'on the', 'to a',
      'to an',   'to be',  'to the', 'with the',
      'has been'
    );
  } ## end else [ if ( $opt{language} eq...)]
  ...
  my @top100Phrasen2;
  foreach my $phrase (
    sort { $zweiWortPhrasen{$b} <=> $zweiWortPhrasen{$a} }
    keys %zweiWortPhrasen
      )
  {
...
sub statistics {
  ...
  if ( $opt{language} eq 'EN' ) {
    ...
    # ===Therefore===
    @_
        = qw (hence thus thereby therefore additionally consequently accordingly finally);
    ...
    # ===measured===
    @_
        = qw (measured observed found detected determined yield probed investigated examined);
    foreach $_ (@_) { $altHash{$_} += $h{$_}; }
